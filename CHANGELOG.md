...
- **MAJOR** - Existing configs that predate the `[syscall]` table now use the Linux seccomp-BPF fence's default `preferred` mode on the next `nocklock wrap`. This keeps the fence default-on where seccomp is supported; set `[syscall].enforcement = "off"` to disable it.
