...
events. Optional NockCC cloud dashboard sync stays separate.
...
1. The user runs `nocklock wrap -- claude --dangerously-skip-permissions`.
2. The CLI parses its arguments and loads `.nock/config.toml`, walking up the
   directory tree to find it.
3. The fence engines initialize. If any fence fails to initialize, NockLock
   aborts and fails closed.
4. The secret fence filters environment variables through the pass and block
   lists. When `[secrets]` scan settings are configured, the secret preflight
   then scans the selected paths and the filtered environment. A finding or an
   incomplete scan refuses the launch and is written to the audit chain (see
   "Secret preflight scanning" in the README).
5. With `filesystem.root` configured on Linux, the filesystem fence applies
   Landlock and LD_PRELOAD with libfence_fs.so, then opens a Unix socket for
   events.
...
8. The network fence is configured. In the default proxy mode it starts the
   ...
   helper creates the namespace, installs the default-drop nftables base, and
   ...
   get no path out. The helper then drops `CAP_NET_ADMIN` and `CAP_SYS_ADMIN`
   from all five capability sets, drops to the invoking user and execs the
   agent. That exec is the single spawn of the child. If privilege cannot be
   acquired or a sidecar dies, the fence fails closed.
...
by root-gated CI jobs in `.github/workflows/network-egress.yml`. Each runs with
a `*_REQUIRE=1` gate, so a skipped test fails instead of reporting green.

`netns-foundation` checks that, with the default-drop base installed, a
capability-dropped child is denied loopback, external TCP and UDP/53, with a
no-drop control.

`q6-acceptance` checks that the capped child gets EPERM on nftables, route and
interface mutation, with a privileged-parent control.

`netns-protocol-matrix` checks that allowlisted HTTP and HTTPS succeed end to
end, non-allowlisted HTTP gets a proxy 403, non-allowlisted TLS is closed at
the proxy, no-SNI direct-IP TLS is terminated, the fixed DNS stub answers over
UDP and TCP inside the namespace, direct resolvers get no reply, UDP/443 (QUIC)
and SCTP stay dropped, curl, Node and Python succeed over the TCP-only path,
and proxy death terminates the child.

`netns-composed-default` checks that Landlock required, seccomp required, netns
and the signed audit log compose through a real `nocklock wrap`: one
allowlisted fetch is permitted, one off-allowlist fetch is refused, both land
signed and `nocklock verify --audit` passes.

`egress-helper-install-readme` checks that the shipped installer produces a
sudoers drop-in scoped to exactly `check` and `setup --request-file *`, and
that `nocklock doctor` reports the helper ok.

## Key design decisions

The Architecture Decision Records live in `.claude/decisions/`. In short: Go
was chosen for single-binary distribution and cross-platform support
(ADR-001). The MVP fences use userspace techniques and need no root (ADR-002);
the Linux netns egress fence is the deliberate exception, since it needs a
root-owned helper reachable through a constrained NOPASSWD sudoers grant. The
config is TOML with strict parsing, so unknown keys are errors (ADR-003). The
netns setup request travels in a 0600 file named on argv rather than on stdin,
...
A concurrent writer running as the same uid that swaps a validated ancestor,
or the DB path, for a symlink between validation and use (including after the
inode re-check, before SQLite lazily opens the file).
...
An `os.Root`/`openat` walk from the project root is not enough on its own:
SQLite reopens the database, and its `-wal`/`-shm` siblings, by pathname. On
Linux, keeping the validated directory descriptor open for the Logger's
lifetime and opening `/proc/self/fd/<dirfd>/events.db` does work with WAL
(verified with `modernc.org/sqlite`). macOS has no equivalent, and a symlink
swapped in at the final component would still be followed. That is a
Linux-only mitigation for a race we do not defend against, so it is not built.
If the threat model ever includes a same-uid racer, start from that Linux
path; macOS keeps a documented residual window.
