...
- `NewFence(pass, block []string) *Fence` — constructor
...
- `Match(name string, patterns []string) bool` — checks if an env var name matches any pattern using `filepath.Match` with case-insensitive comparison (lowercase both sides)
...
Uses `filepath.Match` from stdlib (no third-party dependencies). Both the pattern and env var name are lowercased before matching for case-insensitive behavior.
...
2. If no config exists → warn to stderr, run passthrough (no filtering)
...
- Glob matching uses stdlib only (`filepath.Match`)
