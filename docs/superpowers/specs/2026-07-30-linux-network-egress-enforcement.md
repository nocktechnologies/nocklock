...
7. **All-protocol egress: DECIDED.** Base policy is default-drop across all
   ...
   the QUIC-to-TCP fallback for the real agent runtimes (curl, Node, Python HTTP
...
only):
- **(a) Prove the AppArmor cause** via a toggle test: upgrade *indicated* to *proven*
...
- **Phase 0 (next, cheap):** fleet feasibility probe for Q1, a one-file program
  ...
  deny-all build pass, so each row asserts its *specific* outcome):
    - **allowed HTTP(S) over TCP** to an allowlisted host gives POSITIVE success
      (connection completes through the proxy); to a non-allowlisted host gives 403/deny.
    - **DNS** for an allowed name resolves via the stub; for a non-allowed name fails closed.
      **Direct external-resolver bypass** (instrumented UDP *and* TCP queries aimed at an
      off-namespace resolver), then assert no packet reaches the external endpoint and that every
    ...
    - **direct-IP TCP** (no SNI) is redirected to the transparent proxy and fails closed by being
    ...
    - **UDP/QUIC (UDP/443), SCTP, raw IP** get an explicit DENY (**packet dropped**).
    - **QUIC-to-TCP fallback** (curl/Node/Python) still reaches an allowlisted host (Q7 exit criterion).
    - **proxy death mid-session** makes child egress fail closed end-to-end (watchdog kills the child).
...
(429 to 503) during this window, so on any single run one of the two jobs may fail in
...
(QUIC-to-TCP fallback, the remaining Phase-0 exit criterion). The Phase 1 *foundation*
